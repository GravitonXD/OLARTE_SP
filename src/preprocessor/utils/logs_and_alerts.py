...
class Logs:
    ...
    def success_log(self, message, log_directory):
        ...
        action = "a" if path.exists("/data/db/data-collector_logs/success_log.txt") else "w"
        # Create the folder if it does not exist
        makedirs(f"/data/db/{log_directory}/", exist_ok=True)
        with open(f"/data/db/{log_directory}/success_log.txt", action) as success_log:
    ...
    def error_log(self, message, log_directory):
        ...
        self.logs = f"{self.date}, {self.time}, {message}\n"
        # Define the action to be taken (If no file exists, create a new file)
        action = "a" if path.exists("/data/db/data-collector_logs/error_log.txt") else "w"
        # Create the folder if it does not exist
        makedirs(f"/data/db/{log_directory}/", exist_ok=True)
        with open(f"/data/db/{log_directory}/error_log.txt", action) as error_log:
            # Write the log to the file
